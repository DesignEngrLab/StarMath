...
namespace StarMathLib
{
    public static partial class StarMath
    {
        ...
        private static float[,] inverseWithLUResult(float[,] B, int[] permute, int length)
        {
            ...
            {
                B[permute[i], i] = 1.0f / B[permute[i], i];
                for (var j = i + 1; j < length; j++)
                {
                    var sum = 0.0f;
                    for (var k = i; k < j; k++)
                        sum -= B[permute[j], k] * B[permute[k], i];
                    B[permute[j], i] = (sum == 0) ? 0.0f : sum / B[permute[j], j];
                }
            }
        ...
        /// <exception cref="ArithmeticException">LU Decomposition can only be determined for square matrices.</exception>
        private static float[,] LUDecomposition(float[,] A, out int[] permutationVector, int length = -1,
        ...
        private static bool findAndPivotRows(float[,] B, int[] permutationVector, List<int>[] lastZeroIndices, int i,
