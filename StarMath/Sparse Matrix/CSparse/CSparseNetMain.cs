...
namespace StarMathLib.CSparse
{
    internal static class Main
    {
            ...
            int k, int[] xi, double[] x, int[] pinv, bool lo)
        {
            ...
            var top = ApproximateMinimumDegree.Reach(gColPointers, gRowIndices, bColPointers, bRowIndices, n, k, xi, pinv);
            for (int i = top; i < n; i++)
                x[xi[i]] = 0; // clear x
            for (int i = bColPointers[k]; i < bColPointers[k + 1]; i++)
            ...
            for (int px = top; px < n; px++)
            {
                ...
                int p = lo ? gColPointers[J] + 1 : gColPointers[J]; // lo: L(j,j) 1st entry
        ...
        internal static SymbolicFactorization SymbolicAnalysisLDL(CompressedColumnStorage A)
        {
            ...
            for (int k = 0; k < n; k++)
            {
                ...
                for (p = ap[kk]; p < p2; p++)
                {
                    ...
                    var i = Pinv != null ? Pinv[ai[p]] : ai[p];
                    if (i < k)
                    {
            ...
            out CompressedColumnStorage L)
        {
            ...
            for (int k = 0; k < n; k++)
            {
                ...
                for (int p = aColPointers[kk]; p < p2; p++)
                {
                    int i = Pinv != null ? Pinv[aRowIndices[p]] : aRowIndices[p]; // get A(i,k)
                    if (i <= k)
                    {
